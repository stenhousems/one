...
This function will only execute external commands (cmd for short),
...
function, you must fork a child process.
 ...
 (2) The execv is assuming the full path name for the cmd is provided (i.e. the
 	...
 	process). Instead, the full path (/bin/pwd) must be given. To
 	achieve this, use the find_fullpath function implemented in
 	this header file. An incomplete code segement is provided
 	below.
...
This function is used determine if the named command (cmd for short)
...
HINT(s): Use getenv() system function to retrieve the folders defined in the
		...
		The format of the PATH is very simple, the ':'
		character is delimiter, i.e. used to mark the
		beginning and end of a folder defined in the path.

		Write a loop that parses each folder defined in the
...
This function will determine if command (cmd for short) entered in the
shell by the user is a valid builtin command.
...
function:
	...
	- return: SUCCESSFUL if no error occurred during chdir
		  operation, or ERROR if an error condition was
		  generated by chdir operation.
